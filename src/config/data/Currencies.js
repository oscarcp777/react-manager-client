...
export const Currencies = Object.keys(currenciesISO).map((key) =>( {value: key ,label:`${currenciesISO[key].text}-${key}`}));
export const CurrenciesSymbols = Object.keys(currenciesISO).map((key) =>( {value: key ,label:`${currenciesISO[key].symbol} - ${currenciesISO[key].text}`}));
